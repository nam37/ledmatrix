...
export function getMatrixConfig(): MatrixConfig {
  const matrixOptions: MatrixOptions = {
    ...
    hardwareMapping: (process.env.MATRIX_GPIO_MAPPING || 'regular') as GpioMapping,
    brightness: parseInt(process.env.MATRIX_BRIGHTNESS || '80'),
    pwmBits: parseInt(process.env.MATRIX_PWM_BITS || '11'),
  };

  const runtimeOptions: RuntimeOptions = {
    gpioSlowdown: parseInt(process.env.MATRIX_GPIO_SLOWDOWN || '5'),
    dropPrivileges: 0, // Keep privileges for GPIO access
  };
